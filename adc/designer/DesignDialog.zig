...
pub const Impl = struct {
    ...
    fn isMouseOver(ptr: *const anyopaque) bool {
        const self: *const Impl = @ptrCast(@alignCast(ptr));
        return DesignBehaviours.isMouseOver_whResizable(self, self.r1, self.c1, self.r2, self.c2);
    }

    fn handleMouseDown(ptr: *anyopaque, b: SDL.MouseButton, clicks: u8, cm: bool) !?Imtui.Control {
        const self: *Impl = @ptrCast(@alignCast(ptr));
        return DesignBehaviours.handleMouseDown_whResizable(self, b, clicks, cm, self.r1, self.c1, self.r2, self.c2);
    }

    fn handleMouseDrag(ptr: *anyopaque, b: SDL.MouseButton) !void {
        const self: *Impl = @ptrCast(@alignCast(ptr));
        return DesignBehaviours.handleMouseDrag_whResizable(self, b, 0, 0);
    }
    ...
    pub fn adjustRow(self: *Impl, dr: isize) bool {
        return DesignBehaviours.adjustRow(self, 1, self.imtui.text_mode.H - 1, dr);
    }

    pub fn adjustCol(self: *Impl, dc: isize) bool {
        return DesignBehaviours.adjustCol(self, 1, self.imtui.text_mode.W - 1, dc);
    }
...
pub fn create(imtui: *Imtui, root: *DesignRoot.Impl, id: usize, r1: usize, c1: usize, r2: usize, c2: usize, text: []const u8) !DesignDialog {
    ...
    d.describe(root, id, r1, c1, r2, c2, text);
    return .{ .impl = d };
}
