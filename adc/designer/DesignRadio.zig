...
pub const Impl = struct {
    ...
    pub fn describe(self: *Impl, _: *DesignRoot.Impl, _: *DesignDialog.Impl, _: usize, _: usize, _: usize, _: []const u8) void {
        ...
        self.imtui.text_mode.writeAccelerated(r1, c1 + 4, self.label.items, true);
        ...
        if (!self.imtui.focused(self.control())) {
            ...
            .label_edit => {
                ...
                self.imtui.text_mode.cursor_col = c1 + 4 + self.label.items.len;
    ...
    pub fn createMenu(self: *Impl, menubar: Imtui.Controls.Menubar) !void {
        var menu = try menubar.menu("&Radio", 0);

        var edit_label = (try menu.item("&Edit Label...")).shortcut(.@"return", null).help("Edits the radio's label");
        if (edit_label.chosen())
            try self.startLabelEdit();

        try menu.separator();

        var delete = (try menu.item("Delete")).shortcut(.delete, null).help("Deletes the radio");
        if (delete.chosen())
            self.root.designer.removeDesignControlById(self.id);
...
pub fn bufPrintImtuiId(buf: []u8, _: *DesignRoot.Impl, _: *DesignDialog.Impl, id: usize, _: usize, _: usize, _: []const u8) ![]const u8 {
    return try std.fmt.bufPrint(buf, "{s}/{d}", .{ "designer.DesignRadio", id });
}

pub fn create(imtui: *Imtui, root: *DesignRoot.Impl, dialog: *DesignDialog.Impl, id: usize, r1: usize, c1: usize, label: []const u8) !DesignRadio {
    ...
    d.describe(root, dialog, id, r1, c1, label);
    return .{ .impl = d };
}
...
pub const Schema = struct {
    ...
    pub fn bufPrintFocusLabel(self: *const Schema, buf: []u8) ![]const u8 {
        return try std.fmt.bufPrint(buf, "[Radio] {s}", .{self.label});
    }
};
