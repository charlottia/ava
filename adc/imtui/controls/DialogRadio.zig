...
pub const Impl = struct {
    ...
    pub fn describe(self: *Impl, _: *Dialog.Impl, _: usize, r: usize, c: usize, label: []const u8) void {
        ...
        self.imtui.text_mode.write(self.r, self.c, "( ) ");
        if (self.selected)
            self.imtui.text_mode.draw(self.r, self.c + 1, 0x70, .Bullet);
    ...
    fn isMouseOver(ptr: *const anyopaque) bool {
        ...
        return self.imtui.mouse_row == self.r and
            self.imtui.mouse_col >= self.c and self.imtui.mouse_col < self.c + self.label.len + 3;
    }
...
pub fn bufPrintImtuiId(buf: []u8, dialog: *Dialog.Impl, ix: usize, _: usize, _: usize, _: []const u8) ![]const u8 {
    return try std.fmt.bufPrint(buf, "{s}/{s}/{d}", .{ "core.DialogRadio", dialog.ident, ix });
}
