...
fn renderDisplayDialog(self: *Adc) !void {
    ...
    var dialog = try self.imtui.dialog("adc.DisplayDialog", "Display", 22, 60, .centred);

    dialog.groupbox("Colors", 1, 2, 15, 58, 0x70);

    var r1 = try dialog.radio(4, 4, "&1. ");
    ...
    if (self.display_dialog_help_dialog_visible) {
        var help_dialog = try self.imtui.dialog("adc.DisplayDialog.help", "HELP: Display Dialog", 21, 70, .centred);
